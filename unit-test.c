...
int TestInitRT() {
    ...
    for(i=0; i<resultpkt.no_routes; i++) {
      //printf("unit test result pkt route i: %d \n",resultpkt.route[i].dest_id);
...
int TestSplitHorizon() {
    ...
    // printf("resultpkt.route[nbr].next_hop: %d ;  resultpkt.route[nbr].cost:  %d \n",resultpkt.route[nbr].next_hop,resultpkt.route[nbr].cost);
